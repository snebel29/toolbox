...
Run in terminal interactive mode, mount current directory on `/data` within the docker container instance and join container `network` and `pid` namespaces to see and interact with processes and network in the same way as the joined container.
```
$ docker run -it \
	--rm \
	--volume $(pwd):/data \
